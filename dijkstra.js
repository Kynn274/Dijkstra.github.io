...
function delEdge(edge) {
    //delete edge.fromVertex.edges; cạnh được chỉ định sẽ được xóa khỏi mảng các cạnh của đỉnh fromVertex.edges
    for(var i=0; i< edge.fromVertex.edges.length; i++)
        if(edge.fromVertex.edges[i] == edge) {
            edge.fromVertex.edges.splice(i,1);
            break;
        }
    //delete edge.toVertex.edges mở rộng chức năng của đoạn mã trước xoá ko chỉ xoá  từ đỉnh xuất phát mà còn xoá đỉnh đích
...
//Credit hàm kích hoạt downloadFile
...
//hàm để hiển thị và đóng các hộp thoại (dialog) để cập nhật cost của cạnh và nhãn của đỉnh trong  ứng dụng web.
